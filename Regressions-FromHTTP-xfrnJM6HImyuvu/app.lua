...
regressions = {}
regressions.config = require 'config'
regressions.app = {}
...
local function hasSample(TGuid) 
   ...
   local Success, Result = pcall(db.query, db, {sql=' SELECT COUNT(Id) FROM Log', live = true})
   local Count = 0
   if(Success) then
      Count = tonumber(Result[1]["COUNT(Id)"]:nodeValue())
   end
...
-- New for Iguana 6, builds out a tree of all local assets
-- (excluding project.prj, samples.json)
function buildLocalAssetsTree(LocalAssetTree) 
   local LocalAssets = {}
   for Name, Val in pairs(LocalAssetTree) do 
      trace(Name)
      if not (Name == 'project.prj' or Name == 'samples.json') then
         if type(Val) == 'table' then
            LocalAssets[Name] = climbTree(LocalAssetTree[Name])
         end
         if type(Val) == 'string' and not Name:find('::') then
	         LocalAssets[Name] = Val
         end
      end
   end
   return LocalAssets
...
function regressions.app.overloadTranslator(TGuid)
   ...
   local SimpleTree = buildLocalAssetsTree(Tree[Guid])
   SimpleTree["shared"] = climbTree(Tree.shared) 
   SimpleTree["other" ] = climbTree(Tree.other)
   trace(SimpleTree)
